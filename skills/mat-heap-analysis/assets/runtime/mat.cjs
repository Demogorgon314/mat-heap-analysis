...
// src/types.ts
var MatCliError = class extends Error {
  category;
  hint;
  stdoutTail;
  stderrTail;
...
var MAT_OQL_SPEC = {
  ...
  client_input_rules: [
    ...
    "CLI normalizes and wraps the query for MAT command mode.",
...
function normalizeOqlInput(oql) {
...
var CLI_COMMANDS = [
  ...
  },
  {
    ...
    usage: "mat query --heap <path> (--query <oql> | --query-file <file>) [--format txt|html|csv] [--unzip] [--allowed-root <dir>] [--json]",
    examples: [
  ...
  },
  {
    ...
    usage: "mat run <command-name> --heap <path> [--args <text>] [--format txt|html|csv] [--unzip] [--allowed-root <dir>] [--json]",
    examples: [
...
function getCatalog(section = "all") {
  switch (section) {
    case "commands":
...
var envSchema = external_exports.object({
  MAT_ALLOWED_ROOTS: external_exports.string().optional(),
  MAT_HOME: external_exports.string().optional(),
  MAT_LAUNCHER: external_exports.string().optional(),
  JAVA_PATH: external_exports.string().optional(),
  MAT_XMX_MB: external_exports.string().optional(),
  MAT_TIMEOUT_SEC: external_exports.string().optional(),
  MAT_CONFIG_DIR: external_exports.string().optional(),
  MAT_DATA_DIR: external_exports.string().optional(),
  MAT_DEBUG: external_exports.string().optional(),
  MAT_DEBUG_LOG_DIR: external_exports.string().optional(),
  MAT_PRIVACY_MODE: external_exports.string().optional(),
  MAT_OQL_MAX_BYTES: external_exports.string().optional(),
  MAT_RESULT_PREVIEW_LINES: external_exports.string().optional(),
  MAT_STDIO_TAIL_CHARS: external_exports.string().optional()
}).passthrough();
function parseIntInRange(name, value, fallback, min, max) {
...
function normalizeDirectory(inputPath, envName) {
  const absolute = import_node_path.default.resolve(inputPath);
  if (!import_node_fs.default.existsSync(absolute)) {
    throw new Error(`${envName} does not exist: ${absolute}`);
  }
  if (!import_node_fs.default.statSync(absolute).isDirectory()) {
    throw new Error(`${envName} must be a directory: ${absolute}`);
  }
  return import_node_fs.default.realpathSync(absolute);
}
function parseAllowedRoots(rawRoots) {
  if (!rawRoots) {
    return [];
  }
  const parsedRoots = rawRoots.split(",").map((item) => item.trim()).filter((item) => item.length > 0).map((root) => normalizeDirectory(root, "MAT_ALLOWED_ROOTS"));
  return [...new Set(parsedRoots)];
}
function parseAllowedRootsOverride(roots) {
  if (!roots || roots.length === 0) {
    return void 0;
  }
  return [...new Set(roots.map((root) => normalizeDirectory(root, "--allowed-root")))];
}
var DEFAULT_MACOS_MAT_HOME = "/Applications/MemoryAnalyzer.app/Contents/Eclipse";
function inferAllowedRootsFromHeap(heapPath) {
  if (!heapPath || heapPath.trim().length === 0) {
    return void 0;
  }
  return [normalizeDirectory(import_node_path.default.dirname(heapPath), "--heap parent directory")];
}
function inferDefaultMatHome(platform = process.platform, existsSync = import_node_fs.default.existsSync) {
  if (platform !== "darwin") {
    return void 0;
  }
  ...
  return import_node_path.default.resolve(DEFAULT_MACOS_MAT_HOME);
}
function loadConfig(env = process.env, overrides = {}) {
  ...
  const matConfigDir = import_node_path.default.resolve(parsed.MAT_CONFIG_DIR ?? import_node_path.default.join(import_node_os.default.tmpdir(), "mat-config"));
  const matDataDir = import_node_path.default.resolve(parsed.MAT_DATA_DIR ?? import_node_path.default.join(import_node_os.default.tmpdir(), "mat-workspace"));
  import_node_fs.default.mkdirSync(matConfigDir, { recursive: true });
  import_node_fs.default.mkdirSync(matDataDir, { recursive: true });
  const debug = parseBool(parsed.MAT_DEBUG, false);
  const debugLogDir = import_node_path.default.resolve(parsed.MAT_DEBUG_LOG_DIR ?? import_node_path.default.join(import_node_os.default.tmpdir(), "mat-cli-logs"));
  if (debug) {
    import_node_fs.default.mkdirSync(debugLogDir, { recursive: true });
  }
  const matHome = overrides.matHome ? import_node_path.default.resolve(overrides.matHome) : parsed.MAT_HOME ? import_node_path.default.resolve(parsed.MAT_HOME) : inferDefaultMatHome();
  if (matHome && !import_node_fs.default.existsSync(matHome)) {
    throw new Error(`MAT_HOME does not exist: ${matHome}`);
  }
  const matLauncher = overrides.matLauncher ? import_node_path.default.resolve(overrides.matLauncher) : parsed.MAT_LAUNCHER ? import_node_path.default.resolve(parsed.MAT_LAUNCHER) : void 0;
  if (matLauncher && !import_node_fs.default.existsSync(matLauncher)) {
...
function heapBases(heapPath) {
  const base = import_node_path2.default.basename(heapPath);
  const stem = import_node_path2.default.parse(heapPath).name;
  return [.../* @__PURE__ */ new Set([base, stem])];
}
function safeReadDir(directory) {
  try {
    return import_node_fs2.default.readdirSync(directory, { withFileTypes: true });
  } catch {
    return [];
  }
}
function latestByMtime(paths) {
  ...
  return paths.map((item) => ({ item, mtime: import_node_fs2.default.statSync(item).mtimeMs })).sort((left, right) => right.mtime - left.mtime)[0].item;
}
function isRecentEnough(fullPath, startedAtMs) {
  ...
  try {
    return import_node_fs2.default.statSync(fullPath).mtimeMs >= startedAtMs - 2e3;
  } catch {
    return false;
  }
}
function discoverGeneratedNearHeap(heapPath, startedAtMs) {
  const parent = import_node_path2.default.dirname(heapPath);
  ...
  return entries.filter((entry) => {
    ...
    if (entry.name === import_node_path2.default.basename(heapPath)) {
      return false;
    }
    const fullPath = import_node_path2.default.join(parent, entry.name);
    return isRecentEnough(fullPath, startedAtMs);
  }).map((entry) => import_node_path2.default.join(parent, entry.name)).sort();
}
function findQueryCommandText(queryDir, startedAtMs) {
  const pagesDir = import_node_path2.default.join(queryDir, "pages");
  if (!import_node_fs2.default.existsSync(pagesDir) || !import_node_fs2.default.statSync(pagesDir).isDirectory()) {
    return null;
  }
  const candidates = safeReadDir(pagesDir).filter((entry) => entry.isFile() && /^Query_Command[_\d]*\d+\.txt$/.test(entry.name)).map((entry) => import_node_path2.default.join(pagesDir, entry.name)).filter((fullPath) => isRecentEnough(fullPath, startedAtMs));
  return latestByMtime(candidates);
}
function resolveQueryArtifacts(heapPath, startedAtMs) {
  const parent = import_node_path2.default.dirname(heapPath);
  const bases = heapBases(heapPath);
  ...
  for (const entry of safeReadDir(parent)) {
    ...
    if (entry.isFile() && entry.name.endsWith(".zip") && isRecentEnough(fullPath, startedAtMs)) {
  ...
  return {
    queryDir: queryDir ?? (resultTxt ? import_node_path2.default.dirname(import_node_path2.default.dirname(resultTxt)) : null),
    queryZip,
    resultTxt,
    generatedFiles: generated
  };
}
function resolveReportArtifacts(heapPath, startedAtMs) {
  const generated = discoverGeneratedNearHeap(heapPath, startedAtMs);
  const dirs = generated.filter((item) => {
    try {
      return import_node_fs2.default.statSync(item).isDirectory();
    } catch {
      return false;
    }
  });
...
function resolveIndexArtifacts(heapPath) {
  const parent = import_node_path2.default.dirname(heapPath);
  const heapName = import_node_path2.default.basename(heapPath);
  const indexFiles = [];
  ...
  for (const entry of safeReadDir(parent)) {
    ...
    const mtimeMs = import_node_fs2.default.statSync(fullPath).mtimeMs;
...
var ASM_EXPORT = "--add-exports=java.base/jdk.internal.org.objectweb.asm=ALL-UNNAMED";
var AWT_HEADLESS = "-Djava.awt.headless=true";
var PARSE_APP_ID = "org.eclipse.mat.api.parse";
...
function formatOqlForMatCommand(oql) {
  const escaped = oql.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
  return `"${escaped}"`;
}
function buildParseReportCommand(base, reportId, options) {
...
function buildOqlCommand(base, params) {
  const args = buildCommonArgs(base);
...
function buildGenericCommand(base, params) {
  const args = buildCommonArgs(base);
  const commandPart = params.commandArgs ? `${params.commandName} ${params.commandArgs}` : params.commandName;
...
// src/core/debugLog.ts
var import_node_crypto = __toESM(require("node:crypto"), 1);
var import_node_fs3 = __toESM(require("node:fs"), 1);
var import_node_path3 = __toESM(require("node:path"), 1);
function persistDebugLog(params) {
  if (!params.enabled) {
    return;
  }
  import_node_fs3.default.mkdirSync(params.logDir, { recursive: true });
  const fileName = `${Date.now()}-${params.commandName}-${import_node_crypto.default.randomUUID()}.json`;
  const logPath = import_node_path3.default.join(params.logDir, fileName);
...
// src/core/errorClassifier.ts
var WRITE_PERMISSION_PATTERNS = [
  /permission denied/i,
  /access is denied/i,
...
function classifyRunFailure(run, tailChars) {
...
${stderrTail}`;
  if (run.timedOut) {
    return new MatCliError({
      category: "MAT_TIMEOUT",
      message: `MAT process exceeded timeout (${Math.round(run.durationMs / 1e3)}s).`,
...
function resolveMatLauncher(options) {
  if (options.matLauncher) {
    const absolute = import_node_path4.default.resolve(options.matLauncher);
    try {
      return import_node_fs4.default.realpathSync(absolute);
    } catch {
      throw new MatCliError({
        category: "MAT_NOT_FOUND",
  ...
  const pluginDir = import_node_path4.default.join(import_node_path4.default.resolve(options.matHome), "plugins");
  if (!import_node_fs4.default.existsSync(pluginDir) || !import_node_fs4.default.statSync(pluginDir).isDirectory()) {
    throw new MatCliError({
      category: "MAT_NOT_FOUND",
  ...
  const launcherCandidate = import_node_fs4.default.readdirSync(pluginDir).filter((name) => /^org\.eclipse\.equinox\.launcher_.*\.jar$/.test(name)).sort().at(-1);
  ...
  return import_node_path4.default.join(pluginDir, launcherCandidate);
}
function detectJavaVersion(javaPath) {
  const result = (0, import_node_child_process2.spawnSync)(javaPath, ["-version"], {
    encoding: "utf8",
    timeout: 1e4
  });
  if (result.error) {
...
function ensureAllowedHeapPath(heapPath, allowedRoots) {
  ...
  const absoluteInput = import_node_path5.default.resolve(heapPath);
  let canonical;
  try {
    canonical = import_node_fs5.default.realpathSync(absoluteInput);
  } catch {
    throw new MatCliError({
      category: "HEAP_NOT_FOUND",
  ...
  const stat = import_node_fs5.default.statSync(canonical);
  if (!stat.isFile()) {
    throw new MatCliError({
      category: "HEAP_NOT_FOUND",
  ...
  try {
    import_node_fs5.default.accessSync(canonical, import_node_fs5.default.constants.R_OK);
  } catch {
    throw new MatCliError({
      category: "HEAP_NOT_FOUND",
...
function hasWriteAccessNearHeap(heapPath) {
  const parentDir = import_node_path5.default.dirname(heapPath);
  try {
    import_node_fs5.default.accessSync(parentDir, import_node_fs5.default.constants.W_OK);
    return true;
  } catch {
    return false;
  }
...
function prepareHeapForExecution(heapPath, matDataDir) {
  ...
  const stat = import_node_fs6.default.statSync(heapPath);
  const fingerprint = import_node_crypto2.default.createHash("sha256").update(`${heapPath}:${stat.size}:${stat.mtimeMs}`).digest("hex").slice(0, 16);
  const heapFileName = import_node_path6.default.basename(heapPath);
  const stageRoot = import_node_path6.default.join(matDataDir, "staged-heaps", `${import_node_path6.default.parse(heapFileName).name}-${fingerprint}`);
  const stagedHeapPath = import_node_path6.default.join(stageRoot, heapFileName);
  const markerPath = import_node_path6.default.join(stageRoot, ".source.json");
  if (!isFreshStage(stagedHeapPath, stat)) {
    import_node_fs6.default.rmSync(stageRoot, { recursive: true, force: true });
    import_node_fs6.default.mkdirSync(stageRoot, { recursive: true });
    import_node_fs6.default.copyFileSync(heapPath, stagedHeapPath);
...
function isFreshStage(stagedHeapPath, sourceStat) {
  if (!import_node_fs6.default.existsSync(stagedHeapPath)) {
    return false;
  }
  try {
    const stagedStat = import_node_fs6.default.statSync(stagedHeapPath);
...
// src/core/service.ts
var PATH_OPTION_KEYS = /* @__PURE__ */ new Set(["baseline", "snapshot2"]);
var MatService = class {
    ...
    try {
      ...
      const command = buildOqlCommand(
        ...
        },
        {
          ...
          limit: input.limit === void 0 ? void 0 : this.validateBoundedInt(input.limit, input.limit, 1, 1e7, "limit")
        }
      );
  ...
  validateCommandName(commandName) {
    ...
    throw new MatCliError({
      ...
      hint: `Use one of: ${ALLOWED_COMMANDS.join(", ")}`
    });
  }
  ...
  validateBoundedInt(value, fallback, min, max, field) {
  ...
  async executeMat(commandName, command) {
    try {
  ...
  normalizeError(error, fallbackCategory) {
...
function readResultPreview(filePath, lineLimit) {
  let fd;
  try {
    fd = import_node_fs7.default.openSync(filePath, "r");
  } catch {
    return [];
  }
  try {
    ...
    let remainder = "";
    let bytesRead;
    while ((bytesRead = import_node_fs7.default.readSync(fd, buffer)) > 0) {
...
function formatJsonResponse(command, response, durationMs, cliExitCode) {
...
function formatHumanResponse(command, response, durationMs, cliExitCode, overflowStore) {
...
function renderTopLevelHelp(commands) {
  const lines = [
...
function renderUsageError(message, helpText) {
  return [
    `Error: ${message}`,
...
function renderSuccess(command, response) {
  switch (command) {
    case "doctor": {
      const data = response;
      return [
...
function renderPreview(title, preview) {
  if (preview.length === 0) {
    return "";
  }
  return [title + ":", ...preview.map((line) => `  ${line}`)].join("\n");
}
function renderTail(title, text) {
...
function formatFooter(exitCode, durationMs) {
...
`);
  ...
  let finalExitCode = 0;
  const program2 = buildProgram(env, io, overflowStore, options.matServiceDeps, now, (exitCode) => {
...
function buildProgram(env, io, overflowStore, matServiceDeps, now, onExitCode) {
  const program2 = new Command();
  program2.name("mat").description("Agent-friendly Eclipse MAT CLI").helpOption(false).allowExcessArguments(false).showSuggestionAfterError(false).exitOverride().configureOutput({
    writeOut: (text) => io.stdout.write(text),
    writeErr: () => void 0,
    outputError: () => void 0
  });
  const doctor = program2.command("doctor").description("Validate MAT launcher and Java runtime.").helpOption(false);
  ...
  const report = program2.command("report").description("Run a predefined MAT report.").helpOption(false);
  ...
  report.action(async (reportId, opts) => {
    const startedAt = now();
  ...
  const run = program2.command("run").description("Execute a named MAT analysis command.").helpOption(false);
  ...
  run.action(async (commandName, opts) => {
    const startedAt = now();
  ...
  const index = program2.command("index").description("Check index artifacts for a heap dump.").helpOption(false);
...
function addSharedOptions(command, includeAllowedRoot) {
...
function resolveHeapPath(opts) {
  ...
  return typeof heap === "string" ? heap : void 0;
}
function emitResult(io, overflowStore, command, response, durationMs, json) {
  const cliExitCode = response.status === "error" ? mapCliExitCode(response.category) : 0;
  const text = json ? `${formatJsonResponse(command, response, durationMs, cliExitCode)}
...
function emitFailure(io, overflowStore, response, durationMs, json, command, helpText) {
  const cliExitCode = response.status === "error" ? mapCliExitCode(response.category) : 0;
  const text = json ? `${formatJsonResponse(command, response, durationMs, cliExitCode)}
` : response.status === "error" && response.category === "CLI_USAGE" ? `${renderUsageError(response.message, helpText)}
...
function mapCliExitCode(category) {
  switch (category) {
    case "CLI_USAGE":
      return 2;
    case "MAT_NOT_FOUND":
...
function parseCommanderError(error) {
  return new MatCliError({
    category: "CLI_USAGE",
...
function resolveQueryText(query, queryFile) {
  if (query && queryFile) {
    throw new MatCliError({
      category: "CLI_USAGE",
...
function parseKeyValueOptions(items) {
  const options = {};
  for (const item of items) {
...
function normalizeCatalogSection(section) {
...
function isTopLevelJsonCatalog(argv) {
